...
if ! command -v sshpass &> /dev/null; then
    die "sshpass could not be found. Please install sshpass and try again."
fi

OUTPUT_FILE="mtd_backup.tgz"
OPENWRT="root@192.168.10.1"
TMPDIR=$(mktemp -d)
BACKUP_DIR="${TMPDIR}/mtd_backup"
mkdir -p "${BACKUP_DIR}"
SSH_CONTROL="${TMPDIR}/ssh_control"
SSH_PASS="openwrt"  # Set your SSH password here
...
sshpass -p "${SSH_PASS}" ssh -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o "HostKeyAlgorithms=+ssh-rsa" -o "PubkeyAcceptedAlgorithms=+ssh-rsa" -o "ControlMaster=yes" -o "ControlPath=${SSH_CONTROL}" -o "ControlPersist=10" -n -N "${OPENWRT}"

# This is the command we'll use to reuse the master connection
SSH_CMD=(sshpass -p "${SSH_PASS}" ssh -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o "HostKeyAlgorithms=+ssh-rsa" -o "PubkeyAcceptedAlgorithms=+ssh-rsa" -o "ControlMaster=no" -o "ControlPath=${SSH_CONTROL}" -n "${OPENWRT}")
