...
/* This handles kernel version compatibility for phy_device members */
...
static int an8801_led_init(struct phy_device *phydev)
{
	...
	for (led_id = AIR_LED0; led_id < MAX_LED_SIZE; led_id++) {
		...
		if (led_cfg[led_id].en == LED_ENABLE) {
			data = air_buckpbus_reg_read(phydev, AN8801_PBUS_GPIO_CTRL1);
			data |= BIT(led_cfg[led_id].gpio);
			ret |= air_buckpbus_reg_write(phydev, AN8801_PBUS_GPIO_CTRL1, data);

			data = air_buckpbus_reg_read(phydev, AN8801_PBUS_GPIO_CTRL2);
			data |= LED_GPIO_SEL(led_id, led_cfg[led_id].gpio);
			ret |= air_buckpbus_reg_write(phydev, AN8801_PBUS_GPIO_CTRL2, data);

			data = air_buckpbus_reg_read(phydev, AN8801_PBUS_GPIO_CTRL3);
			data &= ~BIT(led_cfg[led_id].gpio);
			ret |= air_buckpbus_reg_write(phydev, AN8801_PBUS_GPIO_CTRL3, data);
...
static int an8801_ack_interrupt(struct phy_device *phydev)
{
	...
	air_buckpbus_reg_write(phydev, AN8801_IRQ_CTRL_REG1, AN8801_IRQ_CTRL_VAL1);
	reg_val = air_buckpbus_reg_read(phydev, AN8801_IRQ_CTRL_REG2);
	air_buckpbus_reg_write(phydev, AN8801_IRQ_CTRL_REG2, 0x0);
	air_buckpbus_reg_write(phydev, AN8801_IRQ_CTRL_REG2, reg_val | AN8801_IRQ_TRIGGER_BIT);
	air_buckpbus_reg_write(phydev, AN8801_IRQ_CTRL_REG1, AN8801_IRQ_CTRL_VAL2);
	air_buckpbus_reg_write(phydev, AN8801_IRQ_STATUS_REG, AN8801_IRQ_CLEAR_VAL);
	return 0;
}
...
static int an8801_config_intr(struct phy_device *phydev)
{
	if (phydev->interrupts == PHY_INTERRUPT_ENABLED) {
		air_buckpbus_reg_write(phydev, AN8801_GPIO_IRQ_REG, BIT(AIR_INTERRUPT_GPIO) << 16);
		air_buckpbus_reg_modify(phydev, AN8801_IRQ_ENABLE_REG, 0x1, 0x1);
	} else {
		air_buckpbus_reg_write(phydev, AN8801_GPIO_IRQ_REG, 0x0);
		air_buckpbus_reg_modify(phydev, AN8801_IRQ_ENABLE_REG, 0x1, 0x0);
	}
...
static int an8801_did_interrupt(struct phy_device *phydev)
{
	...
	reg_val = air_buckpbus_reg_read(phydev, AN8801_IRQ_STATUS_REG);

	if (reg_val & AN8801_IRQ_STATUS_MASK)
		return 1;

	return 0;
}

/* Restored for compatibility with kernels < 5.11 */
...
/* Removed #ifdef CONFIG_OF and #else stub */
...
static int an8801sb_config_init(struct phy_device *phydev)
{
	...
	if ((reg_value & MCS_LINK_STATUS_MASK) != 0) {
		ret = air_buckpbus_reg_write(phydev, AN8801_PBUS_SERDES_CTRL1, 0x1801);
		if (ret < 0)
			return ret;
		reg_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_SERDES_CTRL1);
		phydev_dbg(phydev,
			"air_buckpbus_reg_read(0x10220010,0x%x).\n", reg_value);

		ret = air_buckpbus_reg_write(phydev, AN8801_PBUS_SERDES_CTRL2, 0x9140);
		if (ret < 0)
			return ret;
		reg_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_SERDES_CTRL2);
		phydev_dbg(phydev,
	...
	pbus_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_POLARITY);
	pbus_value &= ~0x3;
	pbus_value |= priv->pol;
	ret = air_buckpbus_reg_write(phydev, AN8801_PBUS_POLARITY, pbus_value);
	if (ret < 0)
		return ret;
	...
	air_buckpbus_reg_write(phydev, AN8801_PBUS_EFIFO_CTRL, 0x0f);
	air_buckpbus_reg_write(phydev, AN8801_PBUS_EFIFO_PARAM, 0x0a0a0404);
...
int an8801_set_polarity(struct phy_device *phydev, int tx_rx)
{
	...
	pr_notice("\n[Write] Polarity %s\n", tx_rx_string[tx_rx]);
	pbus_data = (air_buckpbus_reg_read(phydev, AN8801_PBUS_POLARITY) &
				(~(BIT(0) | BIT(1))));
	pbus_data |= (BIT(4) | tx_rx);
	ret = air_buckpbus_reg_write(phydev, AN8801_PBUS_POLARITY, pbus_data);
	if (ret < 0)
		return ret;
...
int an8801_info_show(struct seq_file *seq, void *v)
{
	struct phy_device *phydev = seq->private;
	unsigned int tx_rx =
		(air_buckpbus_reg_read(phydev, AN8801_PBUS_POLARITY) & 0x3);
	u32 pbus_data = 0;
	int reg = 0;
	...
	pbus_data = air_buckpbus_reg_read(phydev, AN8801_PBUS_SERDES_STATUS);
	...
	pbus_data = air_buckpbus_reg_read(phydev, AN8801_PBUS_HW_STRAP);
...
#ifdef AN8801SB_DEBUGFS
	reg_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_HW_STRAP);
	if (0 == (reg_value &
		...
		if (ret < 0) {
			...
			/* Do not kfree(priv), it is devm_ allocated */
			return ret;
		}
	} else {
...
static int an8801sb_read_status(struct phy_device *phydev)
{
	...
	if (phydev->link == LINK_DOWN) {
		...
		usleep_range(10000, 11000);        /* delay 10 ms */
		reg_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_SERDES_CTRL1);
		reg_value &= 0x7fff;
		air_buckpbus_reg_write(phydev, AN8801_PBUS_SERDES_CTRL1, reg_value);

		reg_value = air_buckpbus_reg_read(phydev, AN8801_PBUS_SERDES_CTRL2);
		reg_value |= AN8801SB_SGMII_AN0_ANRESTART;
		air_buckpbus_reg_write(phydev, AN8801_PBUS_SERDES_CTRL2, reg_value);
	}
	...
	if (prespeed != phydev->speed && phydev->link == LINK_UP) {
		...
		while (an_retry > 0) {
			usleep_range(1000, 1500);       /* delay 1 ms */
			reg_value = air_buckpbus_reg_read(
				phydev, AN8801_PBUS_SERDES_STATUS);
		...
		if (prespeed == SPEED_1000) {
			air_buckpbus_reg_write(
				phydev, AN8801_PBUS_SERDES_CTRL1, 0xd801);
		} else if (prespeed == SPEED_100) {
			air_buckpbus_reg_write(
				phydev, AN8801_PBUS_SERDES_CTRL1, 0xd401);
		} else {
			air_buckpbus_reg_write(
				phydev, AN8801_PBUS_SERDES_CTRL1, 0xd001);
		}
...
static int an8801r_read_status(struct phy_device *phydev)
{
	...
	if (prespeed != phydev->speed && phydev->link == LINK_UP) {
		...
		if (prespeed == SPEED_1000) {
			data = air_buckpbus_reg_read(phydev, AN8801_PBUS_RGMII_CTRL);
			data |= BIT(0);
			air_buckpbus_reg_write(phydev, AN8801_PBUS_RGMII_CTRL, data);
		} else {
			data = air_buckpbus_reg_read(phydev, AN8801_PBUS_RGMII_CTRL);
			data &= ~BIT(0);
			air_buckpbus_reg_write(phydev, AN8801_PBUS_RGMII_CTRL, data);
